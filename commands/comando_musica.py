        ...
        for doc in db.collection('Playlist').get(): 
            for i in sorted(doc.to_dict()): 
                for fileName in os.listdir(r'./Musicas'):
                    if fileName[0:2] == ('0'+str(int(doc.to_dict()[i])) if len(str(int(doc.to_dict()[i]))) == 1 else str(int(doc.to_dict()[i]))): 
                        musicas += f'\n➤ *{fileName[5:-4]}*'
    ...
    # Retorna e Adiciona o numero da musica
        ...
        try:
            if 0 < int(enviado.replace('add', '')) <= 25:
                ...
                for fileName in os.listdir(r'./Musicas'):
                    if fileName[0:2] == ('0'+str(int(enviado.replace('add', ''))) if len(str(int(enviado.replace('add', '')))) == 1 else str(int(enviado.replace('add', '')))): 
